...
1. 保存当前 `.env` 备份。
2. 切换到清单中的镜像仓库和镜像标签。
3. 先读取每个业务镜像的远端 Docker digest，与本机同一镜像的 digest 比对；未变化的服务跳过，变化或缺失的服务才执行单服务 `docker pull`。Docker 自动复用本机已有层，只下载缺失的变化层。跨多个版本更新时直接校准到目标版本，不要求逐版更新。
4. 使用原有数据卷重启服务，并核验容器实际 image ID 与刚校准的镜像一致。
5. 检查前端健康状态。
6. 成功后写入新的版本号和构建号。
...
`GET`/`HEAD` 拉取请求，写入只允许服务器本机通过 SSH 导入，不需要在客户电脑保存仓库密码。旧版
客户端在升级到新维护程序前会暂缓运行时镜像同步，避免旧更新器执行全量 `compose pull`；新程序
接管后再按 digest 逐服务校准。
