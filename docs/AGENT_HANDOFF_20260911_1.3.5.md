...
> 状态更新：本项已完成，实施说明与验收清单见
> `docs/RELEASE_1.3.5_ENTITLEMENTS_AND_APPROVAL_FIX.md`。该批次同时修复了
> `/admin/entitlements` 接口从未注册（页面必然空白）、注册申请待审批被显示为“正常”、
> 调整套餐报“请在统一认证服务中修改”，以及云端单用户功能授权未镜像到本机等问题。
> 版本号仍为 `1.3.5`，发布时需重新构建后端镜像。
...
> 状态更新：完整验收清单（含本轮新增功能项）见
> `docs/RELEASE_1.3.5_ENTITLEMENTS_AND_APPROVAL_FIX.md` 第 6 节，共 21 项。
