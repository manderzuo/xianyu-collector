...
function Invoke-WslCommand([string[]]$Arguments, [string]$Label) {
    ...
    $stdoutPath = Join-Path ([IO.Path]::GetTempPath()) ("xianyu-wsl-out-{0}.bin" -f ([guid]::NewGuid().ToString('N')))
    $stderrPath = Join-Path ([IO.Path]::GetTempPath()) ("xianyu-wsl-err-{0}.bin" -f ([guid]::NewGuid().ToString('N')))
    try {
        $process = Start-Process -FilePath 'wsl.exe' -ArgumentList $Arguments -RedirectStandardOutput $stdoutPath -RedirectStandardError $stderrPath -WindowStyle Hidden -Wait -PassThru
        $exitCode = $process.ExitCode
        foreach ($outputPath in @($stdoutPath, $stderrPath)) {
            if (-not (Test-Path -LiteralPath $outputPath -PathType Leaf)) { continue }
            $text = Convert-NativeBytesToText ([IO.File]::ReadAllBytes($outputPath))
        ...
        Write-Host "[xianyu] WSL command could not be started: $($_.Exception.Message)" -ForegroundColor Yellow
        return 1
    } finally {
