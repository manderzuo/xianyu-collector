...
if (-not (Test-Path -LiteralPath $EnvFile)) {
    # The updater can be launched directly from a desktop shortcut or by an
    # older package before the first installer run has created app\.env.
    # Update checking is optional and must not turn that recoverable state into
    # a startup error. The normal installer remains responsible for creating
    # the real environment file and secrets.
    Write-XianyuLog -LogPath $LogPath -Message "check_skipped reason=environment_missing path=$EnvFile"
    exit 0
}
