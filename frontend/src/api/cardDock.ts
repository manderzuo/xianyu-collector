...
export const getCardSources = (): Promise<CardDockResponse<CardSource[]>> => {
  return get(`${CARD_DOCK_PREFIX}/sources`).then((response: any) => {
...
export const getCardGoods = (
  ...
  return get(`${CARD_DOCK_PREFIX}/goods`, {
    ...
    const payload = response?.data
    if (Array.isArray(payload)) return { ...response, data: { data: payload, total: payload.length, current_page: page } }
    const rows = payload?.data || payload?.items || payload?.list || []
    return { ...response, data: { ...(payload || {}), data: Array.isArray(rows) ? rows : [], current_page: payload?.current_page ?? payload?.page ?? page } }
  })
}
...
export const getCardGoodsDetail = (
  ...
  }).then((response: any) => ({ ...response, data: response?.data?.item || response?.data || null }))
}
...
export const getCardGoodsStock = (
  ...
  return get(`${CARD_DOCK_PREFIX}/goods/${goodsId}/stock`, {
    params: { source_code: sourceCode },
  }).then((response: any) => ({ ...response, data: response?.data || { subs: [] } }))
}
