...
const unwrapRows = (response: any): { rows: any[]; total: number; page: number; page_size: number } => {
  ...
  return {
    ...
    total: Number(payload?.total ?? (Array.isArray(rows) ? rows.length : 0)),
...
export const getPersonalBlacklist = (params: {
  ...
  return get(`${PREFIX}/personal?${searchParams.toString()}`).then((response: any) => {
    const result = unwrapRows(response)
    return { success: Boolean(response?.success ?? true), data: result.rows, total: result.total, page: result.page, page_size: result.page_size }
  })
}
...
export const getPlatformBlacklist = (params: {
  ...
  searchParams.append('page', String(params.page || 1))
  searchParams.append('page_size', String(params.page_size || 20))
  return get(`${PREFIX}/platform?${searchParams.toString()}`).then((response: any) => {
    const result = unwrapRows(response)
    return { success: Boolean(response?.success ?? true), data: result.rows, total: result.total, page: result.page, page_size: result.page_size }
  })
}
