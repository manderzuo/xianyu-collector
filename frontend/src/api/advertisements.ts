...
export const getPublicAds = async (): Promise<ApiResponse & { data?: PublicAdsResponse }> => {
  const response = await get<ApiResponse<PublicAdsResponse | { items?: Advertisement[]; list?: Advertisement[] }>>(`${PREFIX}/public`)
  const payload = response.data
  if (payload && !Array.isArray(payload) && 'carousel' in payload && 'text' in payload) {
    return { ...response, data: payload as PublicAdsResponse }
  }
  const rows = payload && !Array.isArray(payload) ? (payload.items || payload.list || []) : []
  return {
    ...response,
    data: {
      carousel: rows.filter((item) => item.ad_type === 'carousel'),
      text: rows.filter((item) => item.ad_type === 'text'),
    },
  }
}
