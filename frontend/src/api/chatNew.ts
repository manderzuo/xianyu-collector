...
export interface CustomerOrder {
  ...
  delivery_send_status?: 'success' | 'failed' | 'sending' | null
...
export const getChatAccounts = async (page = 1, pageSize = 20): Promise<ChatAccountsPage> => {
  const res = await get<{ success: boolean; data: ChatAccount[] | { items?: ChatAccount[]; list?: ChatAccount[]; total?: number; hasMore?: boolean }; total?: number; hasMore?: boolean }>(
    `${PREFIX}/accounts?page=${page}&page_size=${pageSize}`,
  )
  const payload = Array.isArray(res.data) ? res.data : (res.data || {})
  let data = Array.isArray(payload) ? payload : (payload.items || payload.list || [])
  let total = res.total ?? (Array.isArray(payload) ? data.length : payload.total) ?? data.length
  let hasMore = res.hasMore ?? (!Array.isArray(payload) && payload.hasMore) ?? false

  // 迁移后的数据库账号不依赖旧版 chat-new 记录；没有旧记录时直接使用当前账号表，
  // 这样在线聊天仍然能显示已接入账号，并在没有会话时给出明确的空状态。
  if (data.length === 0) {
    const accountsRes = await get<{ success: boolean; data?: { items?: Array<{ id: number; account_name?: string; note?: string; status?: string; online?: boolean }> } }>(
      `/api/v1/accounts?page=${page}&page_size=${pageSize}`,
    )
    const accountRows = accountsRes.data?.items || []
    data = accountRows.map((account) => ({
      account_id: String(account.id),
      display_name: account.account_name || String(account.id),
      remark: account.note || '',
      // 连接状态必须使用连接服务的真实结果，账号“启用”不等于已经在线。
      connected: Boolean(account.online),
...
export const getConversations = async (
  ...
  const rawPayload = res.data as unknown
  const payload = Array.isArray(rawPayload)
    ? { conversations: rawPayload as Conversation[] }
    : (rawPayload || {}) as { conversations?: Conversation[]; items?: Conversation[]; list?: Conversation[]; hasMore?: boolean; nextCursor?: number | null }
  const data = {
    conversations: payload.conversations || payload.items || payload.list || [],
...
export const getMessages = async (
  ...
  const rawPayload = res.data as unknown
  const payload = Array.isArray(rawPayload)
    ? { messages: rawPayload as ChatMessage[] }
    : (rawPayload || {}) as { messages?: ChatMessage[]; items?: ChatMessage[]; list?: ChatMessage[]; hasMore?: boolean; nextCursor?: number | null }
  return {
    messages: payload.messages || payload.items || payload.list || [],
    hasMore: !!payload.hasMore,
    nextCursor: payload.nextCursor ?? null,
  }
}
...
export const getCustomerOrders = async (
  ...
  const rawPayload = res.data as unknown
  if (Array.isArray(rawPayload)) return rawPayload as CustomerOrder[]
  const payload = (rawPayload || {}) as { items?: CustomerOrder[]; list?: CustomerOrder[]; orders?: CustomerOrder[] }
  return payload.items || payload.list || payload.orders || []
}
...
export const getQuickPhrases = async (): Promise<QuickPhrase[]> => {
  ...
  const payload = Array.isArray(res.data) ? res.data : (res.data as unknown as { items?: QuickPhrase[]; list?: QuickPhrase[] } | undefined)
