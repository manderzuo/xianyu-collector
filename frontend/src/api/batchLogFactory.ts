...
function normalizeBatchPage<TBatch>(response: any, params?: BatchListQuery): BatchListResponse<TBatch> {
  ...
  const pageSize = Number(payload?.page_size ?? params?.page_size ?? 20)
...
export function createBatchLogApi<TBatch, TDetail>(
  ...
  const getBatches = (params?: BatchListQuery) =>
    get<any>(`${batchesUrl}${buildBatchesQuery(params)}`).then((response) => normalizeBatchPage<TBatch>(response, params))
...
export function createUnifiedBatchLogApi<TBatch, TDetail>(
  ...
  const getBatches = async (params?: BatchListQuery): Promise<BatchListResponse<TBatch>> => {
    ...
    return normalizeBatchPage<TBatch>(response, params)
  }
