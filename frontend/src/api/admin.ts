...
const responseRows = (response: any): any[] => {
  ...
  const rows = payload?.items || payload?.list || payload?.records || payload?.logs || payload?.users || []
  return Array.isArray(rows) ? rows : []
}

const responseTotal = (response: any, rows: any[]) => {
...
export const getUsers = async (params?: { page?: number; pageSize?: number; username?: string }): Promise<{ success: boolean; data?: User[]; total?: number; message?: string }> => {
  ...
  const result = await get<any>(`${ADMIN_PREFIX}/users?${query.toString()}`)
  if (!result.success) {
    return { success: false, data: [], total: result.total, message: result.message }
  }
  const users: User[] = responseRows(result).map((row) => {
    const item = row?.payload && typeof row.payload === 'object' ? { ...row.payload, ...row } : row
    return mapAdminUser({ ...item, id: item.id ?? item.user_id ?? 0, username: item.username ?? item.name ?? '未命名用户', is_admin: Boolean(item.is_admin ?? item.role === 'admin') })
  })
  return { success: true, data: users, total: responseTotal(result, users), message: result.message }
}
...
export const getSystemLogs = async (params?: { page?: number; limit?: number; level?: string }): Promise<{ success: boolean; data?: SystemLog[]; total?: number }> => {
  ...
  const result = await get<any>(`${ADMIN_PREFIX}/logs?${query.toString()}`)
  // 后端返回 { logs: [...] } 格式，转换为 SystemLog 数组
  const logs: SystemLog[] = responseRows(result).map((row, index) => {
    const log = typeof row === 'string' ? row : String(row?.message ?? row?.note ?? row?.payload?.message ?? '')
    return {
    id: String(index),
  ...
  return { success: Boolean(result.success ?? true), data: logs, total: responseTotal(result, logs) }
}
...
export const getRiskLogs = async (params?: {
  ...
  // 转换后端格式为前端格式
  const logs: RiskLog[] = responseRows(result).map(item => ({
    id: String(item.id),
  ...
  return { success: Boolean(result.success), data: logs, total: responseTotal(result, logs), message: result.message }
}
...
export const getAccountLoginLogs = async (params?: {
  ...
  return {
    success: Boolean(result.success),
    data: rows,
    total: responseTotal(result, rows),
    message: result.message,
  }
}
...
export const getAdminStats = async (): Promise<{ success: boolean; data?: AdminStats }> => {
  try {
    const response = await get<ApiResponse<AdminStats>>(`${ADMIN_PREFIX}/stats`)
...
export const getTodayStats = async (): Promise<{ success: boolean; data?: TodayStats }> => {
  try {
    const response = await get<ApiResponse<TodayStats>>(`${ADMIN_PREFIX}/stats/today`)
    return { success: response.success, data: response.data }
  } catch {
    return { success: false }
  }
