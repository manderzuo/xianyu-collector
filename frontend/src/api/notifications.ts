...
export const getNotificationChannels = async (): Promise<{ success: boolean; data?: NotificationChannel[] }> => {
  const result = await get<any>(CHANNEL_PREFIX)
  const payload = result?.data ?? result
  const rows: BackendChannel[] = Array.isArray(payload)
    ? payload
    : (payload?.items || payload?.list || payload?.records || [])
  const channels: NotificationChannel[] = rows.map((item) => {
  ...
  return { success: Boolean(result?.success ?? true), data: channels }
}
...
export const getMessageNotifications = async (): Promise<{ success: boolean; data?: MessageNotification[] }> => {
  const result = await get<any>(MESSAGE_PREFIX)
  const payload = result?.data ?? result
  const source = Array.isArray(payload)
    ? payload
    : (payload?.items || payload?.list || payload?.records || payload)
  const notifications: MessageNotification[] = []
  if (Array.isArray(source)) {
    for (const item of source as BackendNotification[]) {
      notifications.push({
        id: item.id,
        cookie_id: String((item as any).cookie_id ?? (item as any).account_id ?? ''),
    ...
    for (const [cookieId, channelList] of Object.entries(source)) {
      if (!Array.isArray(channelList)) continue
      for (const item of channelList as BackendNotification[]) {
        notifications.push({
          id: item.id,
          cookie_id: cookieId,
          channel_id: item.channel_id,
  ...
  return { success: Boolean(result?.success ?? true), data: notifications }
}
