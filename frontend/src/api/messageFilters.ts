...
export async function getMessageFilters(accountId?: string): Promise<MessageFilter[]> {
  ...
  const result = await get<{ success: boolean; data: MessageFilter[] | { items?: MessageFilter[]; list?: MessageFilter[] } }>(PREFIX, { params })
  if (Array.isArray(result?.data)) return result.data
  return result?.data?.items || result?.data?.list || []
}
