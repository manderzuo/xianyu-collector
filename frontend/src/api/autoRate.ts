...
const AUTO_RATE_PREFIX = '/api/v1/cookies'
const BATCH_RATE_PREFIX = '/api/v1/auto-rate'
...
export const getAutoRateConfig = (accountId: string): Promise<{ success: boolean; data: AutoRateConfig }> => {
  return get(`${AUTO_RATE_PREFIX}/${accountId}/auto-rate`)
}
...
export const updateAutoRateConfig = (
  ...
  return put(`${AUTO_RATE_PREFIX}/${accountId}/auto-rate`, config)
}
...
export const batchRateOrders = (accountIds: string[]): Promise<BatchRateResponse> => {
  return post(`${BATCH_RATE_PREFIX}/batch-rate`, { account_ids: accountIds }, { timeout: 600000 })
}
