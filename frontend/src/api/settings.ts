...
const BOOLEAN_SYSTEM_SETTING_KEYS = ['registration_enabled', 'login_captcha_enabled', 'smtp_use_tls', 'smtp_use_ssl', 'runtime.is_exe_mode', 'account.face_verify_timeout_disable', 'proxy.enabled']
...
export const getSystemSettings = async (): Promise<{ success: boolean; data?: SystemSettings }> => {
  const response = await get<ApiResponse<{ items?: Array<Record<string, unknown>> }>>(SYSTEM_SETTINGS_PREFIX)
  const data = Object.fromEntries((response.data?.items || []).flatMap((item) => {
    const key = typeof item.setting_key === 'string' ? item.setting_key : undefined
    return key ? [[key, item.setting_value]] : []
  }))
  return { success: response.success, data: convertSystemSettings(data) }
}

export const getPublicSystemSettings = async (): Promise<{ success: boolean; data?: SystemSettings }> => {
  const response = await get<ApiResponse<Record<string, unknown>>>(PUBLIC_SYSTEM_SETTINGS_PREFIX)
  return { success: response.success, data: convertSystemSettings(response.data || {}) }
}
...
export const getUserSetting = async (key: string): Promise<{ success: boolean; value?: string }> => {
  try {
    const response = await get<{ success?: boolean; data?: { value?: string | boolean | number | null }; value?: string | boolean | number | null }>(`/api/v1/user-settings/${key}`)
    const rawValue = response.data?.value ?? response.value
    return {
      success: Boolean(response.success ?? true),
      // MySQL JSON、旧版数据和不同驱动可能分别返回 string/boolean/number；
      // 设置比较统一按字符串处理，避免已同意的免责声明每次刷新再次弹出。
      value: rawValue === null || rawValue === undefined ? undefined : String(rawValue),
    }
  } catch {
