...
export function MessageFilterFormModal({
  ...
  useEffect(() => {
    ...
    setSingleAccountId(initialFilter?.account_id || selectedAccount || (accounts.length === 1 ? accounts[0].id : ''))
    setSelectedFilterTypes([])
    setSelectedAccountIds([])
  }, [accounts, initialFilter, mode, selectedAccount])
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    try {
      const result = await onSubmit({
        accountId: isEditMode ? selectedAccount : singleAccountId,
                ...
                {isEditMode ? (
                  ...
                  <Select
                    ...
                    options={accounts.map((account) => ({
                      value: account.id,
                      label: getAccountDisplayText(account),
                      key: account.pk?.toString() || account.id,
                    }))}
                    placeholder="选择所属账号"
                  />
