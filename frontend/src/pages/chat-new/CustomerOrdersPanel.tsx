      ...
      <div className="flex-1 overflow-y-auto p-2 space-y-2">
        {!activeCid ? (
          ...
          <div key={order.order_no} className="rounded-lg border border-gray-200 dark:border-gray-700 p-2.5 text-xs">
            ...
            {SHIPPABLE_STATUSES.includes(order.status) && (
              <button onClick={() => onDeliver(order.order_no)} disabled={anyActionRunning || order.card_only_delivered || order.delivery_send_status === 'success' || order.delivery_send_status === 'sending'} className="mt-1.5 w-full flex items-center justify-center gap-1 py-1.5 rounded border border-blue-300 text-blue-600 hover:bg-blue-50 disabled:bg-gray-100 disabled:text-gray-400 disabled:cursor-not-allowed">
                {deliveringOrderNo === order.order_no ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Truck className="w-3.5 h-3.5" />}
                {order.delivery_send_status === 'sending' ? '卡券发送中' : (order.card_only_delivered || order.delivery_send_status === 'success' ? '卡券已发送' : '发卡发货')}
              </button>
            )}
          </div>
        ))}
