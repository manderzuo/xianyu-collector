...
export function Accounts() {
  ...
  const [showTokenModeHint, setShowTokenModeHint] = useState(() => !isTokenModeHintDismissed())
  const [autoPolishLoadingId, setAutoPolishLoadingId] = useState<string | null>(null)
  ...
  const [contentDetailAccount, setContentDetailAccount] = useState<AccountWithKeywordCount | null>(null)
  const [contentDetail, setContentDetail] = useState<RewriteAccountContentDetail | null>(null)
  const [contentDetailLoading, setContentDetailLoading] = useState(false)
  const [contentSyncing, setContentSyncing] = useState(false)
  const [contentDetailError, setContentDetailError] = useState('')
  ...
  const openContentDetail = async (account: AccountWithKeywordCount) => {
    ...
    try {
      const result = await getRewriteAccountContentDetail(account.id)
      if (!result.success || !result.data) throw new Error(result.message || '账号内容加载失败')
      setContentDetail(result.data)
    } catch (error) {
      setContentDetailError(error instanceof Error ? error.message : '账号内容加载失败')
    } finally {
      setContentDetailLoading(false)
    }
  }

  const syncContentDetail = async () => {
    if (!contentDetailAccount || contentSyncing) return
    setContentSyncing(true)
    setContentDetailError('')
    try {
      const result = await syncRewriteAccountContent(contentDetailAccount.id)
      if (!result.success) throw new Error(result.message || '同步失败')
      const detail = await getRewriteAccountContentDetail(contentDetailAccount.id)
      setContentDetail(detail.data || null)
      addToast({ type: 'success', message: result.message || '账号内容同步完成' })
    } catch (error) {
      setContentDetailError(error instanceof Error ? error.message : '账号内容同步失败')
    } finally {
  ...
  const handleToggleAutoPolish = async (account: AccountWithKeywordCount) => {
    if (autoPolishLoadingId !== null) return
    const newEnabled = !account.auto_polish
    setAutoPolishLoadingId(account.id)
    try {
      const result = await updateAccountAutoPolish(account.id, newEnabled)
      setAccounts(prev => prev.map(a =>
        a.id === account.id ? { ...a, auto_polish: newEnabled } : a,
      ))
      if (!newEnabled) {
        addToast({ type: 'success', message: '商品自动擦亮已关闭' })
      } else {
        const responseData = result.data as { immediate_run?: { data?: { total_items?: number; failed_count?: number; fresh_success_count?: number; already_polished_count?: number } } } | undefined
        const runData = responseData?.immediate_run?.data
        const didNotExecute = Boolean(runData?.already_polished_count && runData.already_polished_count > 0 && !runData.failed_count && !runData.fresh_success_count)
        const message = didNotExecute
          ? `自动擦亮已请求，但闲鱼未执行本次操作（${runData?.already_polished_count ?? 0} 个商品返回当天已擦亮）`
          : '商品自动擦亮已开启，已立即执行一次，之后每6小时执行'
        addToast({ type: didNotExecute ? 'warning' : 'success', message })
      }
    } catch (error) {
      console.error('更新商品自动擦亮开关失败:', error)
      addToast({ type: 'error', message: '更新商品自动擦亮开关失败' })
    } finally {
      setAutoPolishLoadingId(null)
    }
  }
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="rounded-lg border border-red-200 bg-red-50 px-4 py-3 dark:border-red-500/30 dark:bg-red-500/10">
          <div className="flex items-start gap-3">
            ...
            <button
              ...
              title="关闭提示"
              onClick={() => {
                dismissTokenModeHint()
                setShowTokenModeHint(false)
              }}
              className="shrink-0 rounded-md p-1 text-red-500 transition-colors hover:bg-red-100 hover:text-red-700 dark:hover:bg-red-500/20 dark:hover:text-red-300"
            >
              <X className="h-4 w-4" />
            </button>
          </div>
      ...
      {/* Accounts List */}
      <div className="vben-card flex flex-col h-auto min-h-[400px] lg:h-[calc(100vh-280px)]">
        ...
        <div className="flex-none overflow-x-auto scrollbar-visible lg:flex-1 lg:min-h-0 lg:overflow-y-auto">
          {accountsLoading ? (
            ...
            <table className="table-ios" style={{ minWidth: 'max-content' }}>
              <thead className="sticky top-0 bg-white dark:bg-slate-800 z-10">
                <tr>
                  ...
                  <th className="whitespace-nowrap min-w-[160px] lg:sticky lg:right-0 bg-slate-50 dark:bg-slate-800 lg:z-20">操作</th>
                </tr>
              </thead>
              <tbody>
                {accounts.length === 0 ? (
                  ...
                  <tr key={account.id}>
                    ...
                    <td>
                      <div className="flex items-center gap-1 [&>button]:shrink-0">
                        ...
                        <button
                          ...
                          disabled={autoPolishLoadingId === account.id}
                          className={`inline-flex items-center justify-center w-7 h-7 rounded transition-colors ${
                            autoPolishLoadingId === account.id
                              ? 'bg-fuchsia-100 text-fuchsia-500 cursor-wait dark:bg-fuchsia-900/30 dark:text-fuchsia-300'
                              : account.auto_polish
                          ...
                          title={`商品自动擦亮：${account.auto_polish ? '已开启（每6小时执行，点击关闭）' : '已关闭（点击开启并立即执行一次）'}`}
                        >
                    ...
                    <td className="lg:sticky lg:right-0 bg-white dark:bg-slate-800 lg:z-[5] lg:shadow-[-4px_0_8px_-4px_rgba(0,0,0,0.1)]">
                      <div className="flex items-center gap-1">
                        ...
                        <button
                          onClick={() => void openContentDetail(account)}
                          className="inline-flex items-center gap-1 px-2 py-1 text-xs rounded hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors"
                          title="查看账号内容"
                        >
                          ...
                          <button
                            onClick={(e) => {
                              ...
                              if (moreMenuAccountId === account.id) {
                                ...
                                // 窄屏时菜单不能超出视口，否则下方入口会看得到但无法点击。
                                const menuHeight = 10 * 32 + 8
                                const spaceBelow = window.innerHeight - rect.bottom
                                const top = spaceBelow >= menuHeight + 8
                                  ? rect.bottom + 4
                                  : Math.max(8, rect.top - menuHeight - 4)
                                setMoreMenuPosition({ top, right: window.innerWidth - rect.right })
        ...
        {moreMenuAccountId && (
          <>
            ...
            />
            <div 
              className="fixed z-50 w-40 max-h-[calc(100vh-16px)] overflow-y-auto bg-white dark:bg-slate-800 rounded-lg shadow-lg ring-1 ring-black/5 dark:ring-white/10 py-1"
...
要发送给买家的内容`}</pre>
      ...
      {/* 账号内容详情：与当前数据库中的真实账号、同步状态和商品快照保持一致 */}
      {contentDetailAccount && (
        <div className="modal-overlay" onClick={() => !contentSyncing && setContentDetailAccount(null)}>
          <div className="modal-content max-w-5xl w-full" onClick={(event) => event.stopPropagation()}>
            <div className="modal-header">
              <div>
                <p className="text-xs text-slate-400">账号内容</p>
                <h2 className="modal-title">{contentDetail?.account?.account_name || contentDetailAccount.note || `账号 ${contentDetailAccount.id}`}</h2>
              </div>
              <button onClick={() => !contentSyncing && setContentDetailAccount(null)} className="modal-close"><X className="w-5 h-5" /></button>
            </div>
            <div className="modal-body space-y-4">
              {contentDetailLoading && <div className="py-12 text-center text-slate-500">正在加载账号内容…</div>}
              {contentDetailError && <div className="rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-sm text-red-600">{contentDetailError}</div>}
              {contentDetail && (
                <>
                  <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
                    {[
                      ['商品', contentDetail.content?.products ?? 0],
                      ['订单', contentDetail.content?.orders ?? 0],
                      ['消息', contentDetail.content?.messages ?? 0],
                      ['发布任务', contentDetail.content?.publish_jobs ?? 0],
                      ['Cookie记录', contentDetail.login_state?.cookie_records ?? 0],
                    ].map(([label, value]) => (
                      <div key={String(label)} className="rounded-lg border border-slate-200 dark:border-slate-700 p-3">
                        <p className="text-xs text-slate-500">{label}</p>
                        <p className="mt-1 text-xl font-semibold text-slate-800 dark:text-slate-100">{value}</p>
                      </div>
                    ))}
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-3 rounded-lg bg-slate-50 dark:bg-slate-800/60 p-3 text-sm">
                    <div><span className="text-slate-500">平台账号 ID：</span><span className="font-mono">{contentDetail.account?.goofish_id || '未填写'}</span></div>
                    <div><span className="text-slate-500">登录态：</span><span className={contentDetail.login_state?.has_cookie ? 'text-green-600' : 'text-amber-600'}>{contentDetail.login_state?.has_cookie ? '已接入' : '未接入'}</span></div>
                    <div><span className="text-slate-500">实时连接：</span><span className={contentDetail.connection?.is_connected ? 'text-green-600' : 'text-amber-600'}>{contentDetail.connection?.is_connected ? '在线' : contentDetail.connection?.connection_state === 'reconnecting' ? '重连中' : contentDetail.connection?.connection_state === 'connecting' ? '连接中' : contentDetail.connection?.connection_state === 'expired' ? '会话已过期' : contentDetail.connection?.connection_state === 'im_error' ? 'IM 未连接' : '离线'}</span></div>
                    {/* IM 状态与登录态分开显示：只有聊天链路故障时不应让人以为整个账号失效。 */}
                    <div><span className="text-slate-500">聊天链路：</span><span className={contentDetail.account?.im_status === 'connected' ? 'text-green-600' : 'text-amber-600'}>{contentDetail.account?.im_status === 'connected' ? '正常' : contentDetail.account?.im_status === 'error' ? 'IM Token 获取失败（登录态仍有效）' : contentDetail.account?.im_status === 'expired' ? '连接已失效' : '未知'}</span></div>
                    <div><span className="text-slate-500">同步状态：</span><span>{contentDetail.sync?.status || '未同步'}</span></div>
                  </div>
                  {contentDetail.connection?.last_error && <div className="rounded-lg border border-amber-200 bg-amber-50 dark:border-amber-500/30 dark:bg-amber-500/10 px-3 py-2 text-sm text-amber-700 dark:text-amber-300">连接提示：{contentDetail.connection.last_error}</div>}
                  {contentDetail.sync?.last_error && <div className="rounded-lg border border-amber-200 bg-amber-50 dark:border-amber-500/30 dark:bg-amber-500/10 px-3 py-2 text-sm text-amber-700 dark:text-amber-300">最近同步提示：{contentDetail.sync.last_error}</div>}
                  <div className="border border-slate-200 dark:border-slate-700 rounded-lg overflow-hidden">
                    <div className="px-4 py-3 border-b border-slate-200 dark:border-slate-700 flex items-center justify-between"><div><h3 className="font-semibold text-slate-800 dark:text-slate-100">最近同步商品</h3><p className="text-xs text-slate-500 mt-1">商品管理页可继续编辑、配置默认回复和 AI 提示词。</p></div><button className="btn-ios-secondary btn-sm" onClick={() => { setContentDetailAccount(null); navigate('/items') }}>打开商品管理</button></div>
                    {contentDetail.recent_products?.length ? <div className="divide-y divide-slate-100 dark:divide-slate-700">{contentDetail.recent_products.map((item) => <div key={String(item.id)} className="px-4 py-3 flex items-center justify-between gap-4"><div className="min-w-0"><p className="font-medium truncate text-slate-800 dark:text-slate-100">{item.title || '未命名商品'}</p><p className="text-xs text-slate-500 font-mono mt-1">平台商品 ID：{item.external_id || '-'}</p></div><div className="flex items-center gap-3 text-sm"><span className="text-amber-600">{item.price === null || item.price === undefined ? '-' : `¥${item.price}`}</span><span className="badge-gray">{item.status || '-'}</span></div></div>)}</div> : <div className="px-4 py-8 text-center text-sm text-slate-500">暂无已同步商品，请点击“立即同步”。</div>}
                  </div>
                </>
              )}
            </div>
            <div className="modal-footer">
              <button onClick={() => void syncContentDetail()} disabled={contentSyncing || contentDetailLoading || !contentDetail?.login_state?.has_cookie} className="btn-ios-primary">{contentSyncing ? <Loader2 className="w-4 h-4 animate-spin" /> : <RefreshCw className="w-4 h-4" />}立即同步商品</button>
              <button onClick={() => !contentSyncing && setContentDetailAccount(null)} className="btn-ios-secondary">关闭</button>
            </div>
          </div>
        </div>
      )}
