...
export function AddressPickerModal({ open, currentValue, onSelect, onClose }: AddressPickerModalProps) {
  ...
  const selectAddress = (item: AmapInputTip) => {
    if (!item.id.trim() || !item.adcode.trim() || !item.location.trim()) return
  ...
  return (
    <div className="divide-y divide-slate-100 dark:divide-slate-700 rounded-lg border border-slate-200 dark:border-slate-700 overflow-hidden">
      {items.map((item, index) => {
        const selectable = Boolean(item.id.trim() && item.adcode.trim() && item.location.trim())
        return (
        <button
          type="button"
        ...
        >
          <span className="flex items-start gap-2">
            ...
            <span className="min-w-0">
              ...
              {!selectable && <span className="mt-0.5 block text-xs text-amber-600 dark:text-amber-400">缺少完整行政区划或坐标，不可选择</span>}
            </span>
          </span>
        </button>
        )
