          ...
          {(!isFishShop || form.specifications.length === 0) && (
            <section className="space-y-3">
              ...
              <div className={`grid grid-cols-1 gap-3 ${isFishShop ? 'md:grid-cols-3' : 'md:grid-cols-2'}`}>
                ...
                <div className="input-group"><label className="input-label">线上库存</label><input type="number" className="input-ios" min="1" max="999999" step="1" value={form.quantity} onChange={(event) => update({ quantity: Math.min(999999, Math.max(1, Number(event.target.value) || 1)) })} /><p className="mt-1 text-xs text-slate-400">发布后闲鱼可售数量；普通卖家也可设置多个，成交后不会因库存为1自动售罄。</p></div>
              </div>
              {isFishShop
