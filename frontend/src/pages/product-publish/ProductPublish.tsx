...
export function ProductPublish() {
  ...
  useEffect(() => {
    ...
    getPublishAccountCapability(accountId).then((response) => {
      ...
      // API 和原生 select 的值统一为字符串，避免数字/字符串严格比较导致
      // 已成功检测的账号仍被判断为“尚未确认”。
      setAccountCapability({ ...response.data, account_id: String(response.data.account_id) })
    }).catch((error) => {
      if (capabilityRequestRef.current !== requestId) return
  ...
  useEffect(() => {
    ...
    setForm((current) => current.videos.length === 0 ? current : { ...current, videos: [] })
  ...
  const handlePublish = async () => {
    ...
    if (!accountCapability || accountCapability.account_id !== String(form.account_id)) return addToast({ type: 'error', message: capabilityError || '账号发布能力尚未确认，请重新选择账号' })
    ...
    try {
      ...
      if (response.success) emitLiveRefresh('items')
