 ...
 * 3. 支持按状态筛选
 */
import { CheckCircle, Clock, MinusCircle, Sparkles, XCircle } from 'lucide-react'
...
const summaryCards: BatchLogSummaryCard<PolishBatchDetail>[] = [
  {
    ...
    label: '未执行',
    value: (batch) => batch.skipped_count || 0,
    valueClass: 'text-2xl font-bold text-slate-500 dark:text-slate-400',
  },
  {
    icon: Sparkles,
...
const columns: BatchLogTableColumn<PolishLog>[] = [
  ...
  {
    title: '状态',
    render: (log) => log.status === 'skipped'
      ? renderStatusBadge({ label: '未执行', color: 'text-slate-500 dark:text-slate-400', Icon: MinusCircle })
      : renderSuccessFailedBadge(log.status),
  },
  { title: '执行说明', render: (log) => renderDescriptionCell(log.error_message, 50) },
