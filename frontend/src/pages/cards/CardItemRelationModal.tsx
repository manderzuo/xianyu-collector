...
export function CardItemRelationModal({ cardId, cardName, onClose, onSaved, readonly = false }: CardItemRelationModalProps) {
  ...
  const handleSave = async () => {
    setSaving(true)
    try {
      const result = await updateCardItems(cardId, Array.from(selectedItemIds))
      if (result?.success === false) {
        throw new Error(result.message || '关联商品保存失败')
      }
      addToast({ type: 'success', message: result?.message || '关联商品保存成功' })
      onSaved()
      onClose()
    } catch (error) {
