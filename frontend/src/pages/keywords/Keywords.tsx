...
export function Keywords() {
  ...
  const [imageFormAccountId, setImageFormAccountId] = useState('')
  ...
  useEffect(() => {
    if (!_hasHydrated || !isAuthenticated || !token || !isImageModalOpen) return
    if (imageFormAccountId) {
      loadItems(imageFormAccountId)
    } else {
      setItems([])
    }
  }, [imageFormAccountId, isImageModalOpen, _hasHydrated, isAuthenticated, token])
  ...
  const openAddModal = () => {
    const accountId = selectedAccount || (accounts.length === 1 ? accounts[0].id : '')
    if (!accountId && accounts.length === 0) {
      addToast({ type: 'warning', message: '暂无可选账号，无法添加关键词' })
      return
    }
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    const submitAccountId = editingKeyword ? formAccountId : (formAccountId || selectedAccount)
  ...
  const openImageModal = () => {
    const accountId = selectedAccount || (accounts.length === 1 ? accounts[0].id : '')
    if (!accountId && accounts.length === 0) {
      addToast({ type: 'warning', message: '暂无可选账号，无法添加图片关键词' })
      return
    }
    setImageFormAccountId(accountId)
    setImageKeyword('')
  ...
  const handleImageSubmit = async (e: FormEvent) => {
    e.preventDefault()
    const submitAccountId = imageFormAccountId || selectedAccount
    if (!submitAccountId) {
      addToast({ type: 'warning', message: '请选择所属账号' })
      return
    }
                  ...
                  <tr key={getKeywordUniqueId(keyword)} className={selectedKeywordIds.has(getKeywordUniqueId(keyword)) ? 'bg-blue-50 dark:bg-blue-900/20' : ''}>
                    ...
                    <td>
                      <span className="text-xs text-slate-600 dark:text-slate-300">
                        {getKeywordAccountLabel(keyword)}
                      </span>
                    </td>
              ...
              <div className="modal-body space-y-4 overflow-y-auto">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
                    一行一个关键词，多个关键词会对应同一条回复内容；采用包含匹配，例如“你好”可匹配“你好呀”“你好，在吗”，自动忽略大小写和空格
                  </p>
                </div>
                <div>
          ...
          <div className="modal-content max-w-lg">
            ...
            <form onSubmit={handleImageSubmit} className="flex flex-col flex-1 min-h-0">
              <div className="modal-body space-y-4 overflow-y-auto">
                {/* 所属账号 */}
                <div>
                  <label className="input-label">所属账号 <span className="text-red-500">*</span></label>
                  <Select
                    value={imageFormAccountId}
                    onChange={(value) => {
                      setImageFormAccountId(value)
                      setImageSelectedItemIds([])
                      setImageItemSearchText('')
                    }}
                    options={accounts.map((account) => ({
